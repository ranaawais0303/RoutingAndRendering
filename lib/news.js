...
`;
  ...
  console.log(months, "here are the years");
...
export const getNewsForYear = async (year) => {
  try {
    const result = await db
      ...
      .query(
        "SELECT * FROM news WHERE FORMAT([date], 'yyyy') = @year ORDER BY date DESC"
      );
...
export const getNewsForYearAndMonth = async (year, month) => {
  try {
    const result = await db
      ...
      .query(
        "SELECT * FROM news WHERE FORMAT([date], 'yyyy') = @year AND FORMAT([date], 'mm')= @month ORDER BY date DESC"
      );
